...
10. **The durable mirror writes a bucket once, when it closes; the offsetlog backs it until then.** `flame-stats` is append-only and last-frame-wins per key, so mirroring the bucket still being accumulated into — once per 30s checkpoint, ten times over a 5-minute bucket — leaves nine dead copies retained for `stats_mirror_lifetime`. Measured before the change: **4,279,767 of 5,531,418 bytes (77.4%)** of a real `flame-stats.p0` were frames a later frame had already superseded. `flush_stats_mirror()` therefore holds a bucket's frames until `Stats_Store::is_open_bucket()` says it is closed (`>=`, not equality, so a producer whose clock runs ahead is held too). Until then those frames are durable in the OFFSETLOG instead — a self-pruning ring of at most 60 keyframes, where the mirror retains for twice the stats window, so the open window costs bounded disk in the cheap place. Two consequences the code must keep honoring: the worker's `rehydrate` seam consults the held frames FIRST, or a mid-bucket eviction makes the read-modify-write restart the bucket from zero; and the carry is capped at `MAX_CHECKPOINT_MIRROR_BYTES` and logs what it drops, because the offsetlog bounds keyframe COUNT and not bytes while the per-server aggregates grow with the spoke count. **Revisit if** a snapshot's state ever has no durable home and no natural cap.
