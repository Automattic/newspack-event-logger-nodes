...
| `includes/app/class-{discovery,logger,events,performance}-ci-node.php` | Service CIs (`*_CI_Node`) mounted on `request_graph_ready`; the command-protocol REST surface. `status`/`settings`/`aggregator` are now substrate-owned CIs, and the `servers` CI was replaced by the substrate `vault` CI |
