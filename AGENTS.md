...
| `includes/class-{settings-event-writer,discovery-collector-node}.php` | Hub-side settings-sync + discovery (the node-graph push/discovery path; the old `Server_Registry`/`Settings_Sync`/`Remote_Manager`/`Health_Check_*` were deleted — credentials live in the substrate `Vault`, settings/discovery run as nodes) |
...
| `includes/app/class-{discovery,status,settings,logger,events,aggregator,performance}-ci-node.php` | Service CIs (`*_CI_Node`) mounted on `request_graph_ready`; the command-protocol REST surface (the `servers` CI was removed with the server registry) |
...
| `src/` | React dashboard trees (`overview`, `error-log`, `gyroscope`, `settings`, `requests`) + the `current-request` overlay-tab bundle. Shared dashboard SCSS lives in `src/styles/` (`base.scss` + `_tokens.scss` + `_dashboard-mixins.scss`); each dashboard's `styles/base.scss` `@use`s it |
