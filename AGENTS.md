...
- **Application Node classes resolve by namespace prefix — no per-class registration.** `newspack-event-logger-nodes.php` calls `\Newspack_Nodes\Command_Interpreter_Node::register_namespace('Newspack_Event_Logger_Nodes\\')` (+ the `App\` sub-namespace for the service CIs). `make_node Flame_Builder` resolves `\Newspack_Event_Logger_Nodes\Flame_Builder_Node`, and the palette scans the classmap. A new application node just needs `class Foo_Node extends \Newspack_Nodes\Node` under that namespace + `composer dump-autoload -o`; nothing to register.
