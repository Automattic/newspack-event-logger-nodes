...
final class UninstallCleanupTest extends TestCase {
	...
	public function test_deletes_prefixed_options_and_their_transients_only(): void {
		$GLOBALS['_wp_options'] = [
			'newspack_event_logger_nodes_topologies'                    => [ 't' ],
			'newspack_event_logger_nodes_base_directory'                => '/tmp',
			'_transient_newspack_event_logger_nodes_lock'               => 1,
			'_transient_timeout_newspack_event_logger_nodes_lock'       => 123,
			'other_plugin_option'                          => 'keep',
			'siteurl'                                      => 'https://example.test',
		];

		$deleted = \Newspack_Event_Logger_Nodes\delete_prefixed_options( $this->wpdb(), 'newspack_event_logger_nodes_' );

		$this->assertSame( 4, $deleted );
		$this->assertArrayNotHasKey( 'newspack_event_logger_nodes_topologies', $GLOBALS['_wp_options'] );
		$this->assertArrayNotHasKey( '_transient_newspack_event_logger_nodes_lock', $GLOBALS['_wp_options'] );
		$this->assertArrayNotHasKey( '_transient_timeout_newspack_event_logger_nodes_lock', $GLOBALS['_wp_options'] );
