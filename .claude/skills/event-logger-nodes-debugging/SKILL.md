...
| `trace_callers` | off | A deep backtrace on the start entry's `caller` field, on hook, query and HTTP spans alike, budgeted per hook name, per statement shape and per URL and reset for each job — a stored `true` decodes to `Rule::TRACE_CALLERS_DEFAULT` (20) |
