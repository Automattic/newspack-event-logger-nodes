...
- Changing `Log_Manager`, `Flame_Builder_Node`, `Stats_Store`, `Job_Router_Node`, `Remote_Job_Rewrite_Node`, `Discovery_Collector_Node`, or the ruleset trio (`Rule` / `Rule_Set` / `Rule_Matcher`)
...
- A rule's id is `Rule_Set::id_for( $pattern )`, the pattern's `Log_Manager::url_hash()`. One id per pattern; client-supplied ids are ignored, and every path that accepts outside rules rekeys.
...
`App\MCP_Controller` registers this plugin's one REST route, `POST /wp-json/newspack-event-logger-nodes/v1/mcp`, a JSON-RPC server (protocol `2025-06-18`) wrapping verbs that already exist. It offers ten tools today — seven `performance` reads, `rules_list`, `rules_upsert`, `rules_delete` — declared in the private `TOOLS` map as `{ node, verb, role, summary, args }`.
...
1. Source under `src/{tree-name}/`, with an entry added to `scripts/build.mjs`. Six entries build today: `overview`, `error-log`, `gyroscope`, `requests`, `settings`, `current-request`.
...
3. Use `@wordpress/element` (not direct React) and `@newspack-nodes/runtime` for substrate JS primitives. Shared hooks, helpers, nodes and components come from `@newspack-nodes/shared/...`; there is no local `src/shared/` and no sync step. The one ELN-owned test helper (`renderHook`) lives in `src/test-helpers/`.
