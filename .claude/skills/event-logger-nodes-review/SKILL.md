...
- **A reader's fallback horizon is the writer's TTL.** The fallback for an unfolded hour reads that hour's fine buckets, and the plan's hours span a retention window while `ROLE_URL_FINE` keeps four hours. `Stats_Store::unfolded_hour_buckets()` is the one spelling of it and stops at `ttl_url_fine()`; a diff calling `buckets_in_hour()` from a READ path is asking for keys the schema has discarded, and every one is a certain miss and a full index walk. `roll_up_hours()` folds from the same keys, so nothing behind the horizon was recoverable anyway.
