...
A span is named for its CALLER, never for its host or its table. Both pairs set `l` from `origin_frame( true )`, which climbs past EVERY frame of the class that applied the filter, at `TRANSPORT_ORIGIN_DEPTH` (16) against the `ORIGIN_DEPTH` (8) a hook's own origin takes, because `wpdb::get_row()` reaches the `query` filter through `wpdb::query()` and a fixed skip would land inside `wpdb`. They climb whatever `trace_callers` says: that budget attaches `caller` to hook entries alone, so on a transport span `l` is the only field naming who asked, and a diff coupling the climb to the budget leaves every traced rule's spans reading `wpdb->query` and `WP_Http->request`. And a field nothing renders does not exist: `LogEntriesTable` renders `m`, the origin frame `l` and `caller`, so a new traced field needs its own reader there.
