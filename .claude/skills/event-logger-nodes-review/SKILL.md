...
- **Never read, or remember, a namespace the write path refuses.** `Flame_Builder_Node::mirrors_key()` is the one predicate, and BOTH reader seams ask it: `rehydrate_seam()` drops any key whose namespace `STATS_MIRROR_TOPN` caps at zero (`urls_h`, `urlnames_h`, `lb_h`, the four ranked-list tiers `urlrank`, `urlrank_h`, `urlrank_s` and `urlrank_sh`, and `urltoken`), and the absence closure `arm_stats_reader()` installs holds 0 for it — a read that did not look is no absence to remember. `buffer_mirror_write()` never stored those tiers, so a read could only walk and find nothing. A new derived namespace added to the map at `0` inherits both for free — one added anywhere else does not, and a diff that sets `$store->absence` unconditionally again is the regression to look for.
