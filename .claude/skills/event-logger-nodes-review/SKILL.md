...
Settings live in ONE declarative `Settings_Schema` (`includes/class-settings-schema.php`), one `Field` per setting, from which the Config overlay keys, the admin register-and-render loop, and worker-restart classification (`restart_for()`) all derive. Ten application keys are declared there with their defaults: three rendered checkboxes (`enable_logging`, `log_memory`, `flush_every_line`) and seven overlay-only (`allowed_users`, `rules`, `hook_start_priority`, `custom_colors`, `stats_mirror_node`, `stats_mirror_read_budget_ms`, `recommended_log_events`).
