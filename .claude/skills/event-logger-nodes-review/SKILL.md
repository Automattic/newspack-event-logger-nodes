...
- **The fine tier answers the last hour and feeds the fold. It is not a tier to read old hours from.** Its two consumers are `RECENT_BUCKETS`' last-hour rate and `roll_up_hours()`, and `FINE_TTL_SECONDS` is two hours because that is what those two need. The fallback for an unfolded hour therefore reaches the GRACE hour — the one immediately behind the fine tail — and no further, filtered again to the buckets `ttl_url_fine()` can still hold. `Stats_Store::unfolded_hour_buckets()` is the one spelling of both bounds; a diff calling `buckets_in_hour()` from a READ path is asking for keys the schema has discarded, and every one is a certain miss and a full index walk. `roll_up_hours()` folds from the same keys, so nothing past the grace hour was recoverable anyway.
