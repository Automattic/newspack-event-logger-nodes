...
Settings live in ONE declarative `Settings_Schema` (`includes/class-settings-schema.php`), one `Field` per setting, from which the Config overlay keys, the admin register-and-render loop, and worker-restart classification (`restart_for()`) all derive. Nine application keys are declared there with their defaults: three rendered checkboxes (`enable_logging`, `log_memory`, `flush_every_line`) and six overlay-only (`rules`, `hook_start_priority`, `custom_colors`, `stats_mirror_node`, `stats_mirror_read_budget_ms`, `recommended_log_events`). `allowed_users` is the substrate's key: the admin surface inherits it through `Capabilities::can( MANAGE )`.
...
- **Never read a namespace the write path refuses.** `rehydrate_seam()` drops any key whose namespace `STATS_MIRROR_TOPN` caps at zero (`urls_h`, `urlnames_h`, `lb_h`); `buffer_mirror_write()` never stored them, so a read could only walk and find nothing. A new coarse tier added to the map at `0` inherits this for free — one added anywhere else does not, and that asymmetry is the bug to look for.
