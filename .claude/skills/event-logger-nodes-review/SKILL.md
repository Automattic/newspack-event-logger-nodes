...
Every cap FOLDS its tail rather than dropping it, ranked by traffic, so totals stay exact. The five bucket caps run through `Stats_Store::cap_bucket()` into one synthetic `Stats_Store::OTHER_KEY` (`'Other'`); only the category-SERIES cap passes a reserved row, holding the `total` pseudo-category clear of the ranking and restoring it after. The leaderboard's category cap needs none, because `collision_free_category()` renames an incoming `total` to `total (event)` and keeps that map clear of one; it is applied twice, once as `leaderboard_intent()` writes a bucket and once as `sums_to_display()` merges the window for a reply. The URL-index cap runs through `cap_url_rows()` into TWO overflow rows, `Stats_Store::other_key( $worker )` minting `OTHER_KEY` or `OTHER_WORKER_KEY`, because one row cannot answer a filter about a mixed set.
