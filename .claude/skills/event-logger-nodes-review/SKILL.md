...
| `Stats_Store::MAX_LB_CATEGORIES` | 200 | Categories per leaderboard bucket or URL profile |
| `Stats_Store::ITEM_BUDGET` | 900000 | Bytes any one stored value may take, by its producer's estimate |

**Every value is capped BEFORE it is written (decision 30).** A producer estimates its value from `Stats_Store::overhead()` — a fixed cost per stored part for the configured serializer — plus the `strlen()` of its strings, and caps it to `ITEM_BUDGET`: a URL shard by bytes alone (`cap_url_rows()`), a leaderboard by `MAX_LB_CATEGORIES` and bytes, the `url` blob by a leaderboard cap on its profiles and a lightest-leaf prune of its tree, and a rule's hook list by refusal at `Rule_Set::save()`. A diff that finds a size from a refused write, relies on compression, or serializes a value on the flush path to measure it is the regression to look for; a new namespace needs its cap and a case in `tests/unit/ItemBudgetTest.php`.
...
Raise a cap only against the value size it implies, and never remove the `Other` fold: memcache rejects a value over 1MB, and every cap here exists so no write finds that out.
...
The URL index row is a plain array read through `Stats_Store::ROW_*` — `ROW_COUNT` 0 through `ROW_PATH` 13 — because `serialize()` writes every key name into every row and this is the one namespace whose rows are counted in tens of thousands. **A bare integer literal is forbidden**; the constants are what buy back the readability the shape spends.

Three invariants a diff must keep:

- **The eight fields that ADD come first, in `ROW_SUMS` order**, so one map describes the row's summed half. A ninth summed field appended past index 7 falsifies that; `StatsStoreTest` pins the key sets.
...
- **`ROW_PATH` is the row's one string, and `row_path()` is its one spelling.** It drops the scheme and host only for an https URL under the row's own server, and cuts to `MAX_PATH_BYTES` (1,024). A merge keeps whichever side's path is non-empty; an overflow row's is `''`. A diff writing a host into the row, or reading the host back out of it, breaks decision 30's second rule.
...
Four invariants a diff must keep. **The HTTP pair is deliberately unbalanced**: `WP_Http::request()` short-circuits with a bare `return $pre;` and never fires `http_api_debug`, so `http_start` binds at `PHP_INT_MAX` and opens nothing when `$preempt` is not false. **`query_end()` drains `$wpdb->queries`**, which is why `log_queries` cannot be always-on — anything else reading that array would find it empty. **Instrumentation never CONSTRUCTS the logger**: every instrumentation callback asks `Log_Manager::started_instance()` first, the wrapped listeners included, or a binding that outlives its request builds a logger inside the callback and stamps `process (start)` with the wrong moment. **A scope change unbinds `query_start` and empties both open-span stacks, and `query_end` stays bound once armed**: `SAVEQUERIES` outlives every scope, so the drain must too.

A span is named for its CALLER, never for its host or its table. Both pairs set `l` from `origin_frame( true )`, which climbs past EVERY frame of the class that applied the filter, at `TRANSPORT_ORIGIN_DEPTH` (16) against the `ORIGIN_DEPTH` (8) a hook's own origin takes, because `wpdb::get_row()` reaches the `query` filter through `wpdb::query()` and a fixed skip would land inside `wpdb`. They climb whatever `trace_callers` says, and that budget reaches a transport span as well as a hook — keyed on the statement shape or the URL — so `l` names one frame on every call while `caller` carries the chain on the first few. A diff coupling the climb to the budget leaves every traced rule's spans reading `wpdb->query` and `WP_Http->request`. And a field nothing renders does not exist: `LogEntriesTable` renders `m`, the origin frame `l` and `caller`, so a new traced field needs its own reader there.
...
Settings live in ONE declarative `Settings_Schema` (`includes/class-settings-schema.php`), one `Field` per setting, from which the Config overlay keys, the admin register-and-render loop, and worker-restart classification (`restart_for()`) all derive. Eleven application keys are declared there with their defaults: three rendered checkboxes (`enable_logging`, `log_memory`, `flush_every_line`) and eight overlay-only (`rules`, `hook_start_priority`, `custom_colors`, `stats_mirror_node`, `stats_mirror_read_budget_ms`, `stats_mirror_segment_size`, `stats_mirror_num_segments`, `recommended_log_events`). `allowed_users` is the substrate's key: the admin surface inherits it through `Capabilities::can( MANAGE )`.
...
- **`aggregator` ships no `Remote_Source` node.** The operator wires one per spoke and partition on the console canvas — by hand, or all at once as a `Vault_Group` — with `set_multi_writer true` and an offsetlog scoped by `<topology>`. A diff hardcoding spokes into the file turns a per-deployment fleet into a shipped constant.
...
### 27. The dashboard's mirror read is bounded, and the coarse tiers are not read at all

`Performance_CI_Node::stats_stores()` arms every reader store against the durable stats mirror, so a memcache miss falls through to `Partition_Node::locate_by()`. That walk stops early only once EVERY wanted key is found, so a key ABSENT from the index costs a full pass over the partition's `.idx` — measured at 192ms over 150,000 lines. A cold fold issues 3,456 such batches (sixteen shards by four partitions by twenty-five bucket chunks), and the batch count cannot shrink because the per-shard loop is decision 17's memory bound. A `urls --search` the token index serves walks only the shards its candidates fall in (decision 28); one it cannot serve pays the whole fold. That was the 200-290 second dashboard timeout.

Two rules hold it, and a diff must keep both:

- **One verb, one budget.** `dispatch()` — not `fill()`, because `Mcp_Controller` calls it straight — zeroes `stats_mirror_read_budget_ms` (default 5000, `0` off), and `arm_stats_reader()` stops consulting the mirror once it is spent. This is the same family as `scan_floor()` and `MAX_SCAN_S`: every disk scan in this CI carries a bound, and a new one that walks an index needs its own. The WORKER's seam (`arm_stats_mirror`) is deliberately unbudgeted — it restores its own state rather than answering a poll.
- **Never read, or remember, a namespace the write path refuses.** `Flame_Builder_Node::mirrors_key()` is the one predicate, and BOTH reader seams ask it: `rehydrate_seam()` drops any key whose namespace `Stats_Store::is_derived()` names (`urls_h`, `urlsrv_h`, `lb_h`, the two ranked-list tiers `urlrank_s` and `urlrank_sh`, and `urltoken`), and the absence closure `arm_stats_reader()` installs holds 0 for it — a read that did not look is no absence to remember. `buffer_mirror_write()` never stored those tiers, so a read could only walk and find nothing. A new derived namespace added to that match inherits both for free — one added anywhere else does not, and a diff that sets `$store->absence` unconditionally again is the regression to look for.
- **A per-URL namespace mirrors in FULL, and its bound is `MAX_HELD_FRAMES`, not a rank.** Rank-capping `url_dim` or `url_cat` drops the quiet URLs from the durable mirror permanently, which is only visible once a cache scope moves. What the buffer may HOLD is bounded separately, and its overflow SPILLS to the partition (`spill_over_backstop()`) rather than being dropped — a bound that discards a frame instead of writing it is the regression to look for. It spills a BAND, a tenth of the bound, because the buffer pins at the bound under the very traffic the bound is for and ranking it is a full pass; a diff that spills one frame per write puts that pass on every write. With no partition to spill into it refuses the ARRIVAL and keeps what is held, which is the reading `flush_stats_mirror()` already takes of an unresolved name.

- **The fine tier answers the last hour and feeds the fold. It is not a tier to read old hours from.** Its two consumers are `RECENT_BUCKETS`' last-hour rate and `roll_up_hours()`, and `FINE_TTL_SECONDS` is two hours because that is what those two need. The fallback for an unfolded hour therefore reaches the GRACE hour — the one immediately behind the fine tail — and no further. `Stats_Store::unfolded_hour_buckets()` is the one spelling of it; a diff calling `buckets_in_hour()` from a READ path outside the grace hour is one certain miss per shard per chunk, each a full index walk. **Do not also filter by `ttl_url_fine()`** — that is the MEMCACHE lifetime, and `urls` / `urlsrv` mirror in full for twice the stats window, so an expired bucket is still on disk and the rehydrate seam answers from it. That recovery is exactly what lets the coarse tier stay unmirrored; filtering by the writer's TTL took it away and made an evicted hour permanent loss (v0.88.2, fixed in v0.88.3).
...
- Every test gets one second — `phpunit.xml` sets no `defaultTimeLimit` — and `failOnRisky="true"` makes a breach fail the run rather than warn. Refuse a `#[Medium]` or `#[Large]` added to fit a test that waits in real time: the test pins a clock instead: `Core::$now`, the tick every reader here dates from, and `Core::$clock` or `Event_Framework::$sleep` in the substrate. No class in the suite declares a size.
...
- The URL index stores at TWO resolutions and the coarse one is derived. `urls_h` is folded from `urls` on the writer's flush path, and an hour with no coarse key is answered from its fine buckets only when it is the GRACE hour immediately behind the fine tail — which is what makes a fresh deploy self-healing rather than a hole.
