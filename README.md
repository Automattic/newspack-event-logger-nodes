...
| WordPress | 6.5 |
| PHP | 8.2 |
| `newspack-nodes` | 2.65.12, installed and active |
...
Matching is most-specific-first: a query-bearing pattern (`/jobs/x?job-work`) outranks an exact path (`/about?`), which outranks a prefix (`/blog`). Length breaks ties only *within* a rank, so this is not longest-prefix-wins, and list order never decides the outcome. Comparison is case-insensitive. No rule matched means skip — there is no implicit log-all baseline, so a site that logs everything declares a `/` log rule, which is the whole shipped seed; `/wp-cron.php` and the substrate's own endpoints need no skip, because the logger names each as a worker, and worker traffic stays off the URL table's default rows and out of the global sums.
