...
- **Query and HTTP spans are labelled by CALLER, not by table or host.** `l` is now `origin_frame( 1 )` for both. The `1` is the whole point: `query` and `pre_http_request` are applied from `wpdb` and `WP_Http`, not from application code, so the frame a hook span takes names the transport and is the same string on every call — `origin_frame()` alone would have replaced a varying label with a constant. One frame further out names the code that asked. This is decision 23's own recorded reopen condition, met: on the El Sol save, naming the table said `SELECT wp_posts` ran 156 times and never which code ran it. `sql_label()` and `http_label()` are deleted with their tests.
