...
- **`RequestBuilder` attaches `RequestFlight` as a hidden sibling and adds the compact-summary emit path.** Constructor auto-instantiates the Flight sibling; the overridden `name()` setter cascades to `{patron}:flight` so the topology console's `ls -al` keeps the pair in lockstep, and the overridden `sink()` setter propagates the make_node-installed sink down so Flight's periodic emit reaches the same downstream as the primary doc stream. Adds `set_completed_target( $node_name )` (verb on the `:config` CI) — when set, every completed request gets a secondary `TM_STRUCT` emit alongside the existing full-doc emit, carrying an 11-field HTTP-access-log-style summary (`rid`, `method`, `url`, `start_time`, `end_time`, `duration_ms`, `status_code`, `state`, `error_status`, `remote_addr`, `user_agent`) — schema mirrors the legacy `requests-stream-controller::transform_line` shape. URL clipped to 2000 chars + `...`; user-agent to 500. `inflight_snapshot()` returns the live LruCache as compact-summary rows tagged `state: 'active'` (what RequestFlight emits each tick). `target()` advertises `completed_target` alongside `errors_target` so neither orphans on the topology canvas.
- **Registers `newspack_nodes/expected_log_basenames` filter** so substrate's `Log_Cleaner` (≥ v0.1.32) can auto-delete entire log directories whose basename no longer belongs to any active topology — the orphan-from-topology-change case (e.g. disabling `request-workers` means `flames.log` is no longer produced or read). Filter returns the union of (a) topologies in the application's `topologies` config, and (b) topologies whose worker lock dirs are still on disk (via `Cli::ls_workers()`, which excludes stale lock dirs — a worker that died without cleanup doesn't pin its basenames forever). The per-topology basename map lives in a module-level `NEWSPACK_EVENT_LOGGER_NODES_TOPOLOGY_BASENAMES` constant; update it when topologies are added or their inputs/outputs change.
...
- **`RequestBuilder::inflight_snapshot()` now exposes a `current` field per row** — the request's current activity string, drawn from an explicit `current_hook` slot on the cached request envelope or, failing that, the top of the request's hook stack. Restores parity with the legacy `InflightTracker::get_active` shape (the legacy controller condensed `state` + `what` into the same role) so the M5 deletion of `gyroscope-stream-controller` can land without losing the dashboard's "what is this request doing right now" column.
- **`firehose-workers-and-jobs` + `firehose-workers-only` topologies wire the compact-summary fan-out live.** Adds a `completed:tee` Tee fanning out to two new Partitions, `completed:partition` (`completed.log`) and `gyroscope:partition` (`gyroscope.log`), then points RequestBuilder's `set_completed_target` at the Tee and `set_inflight_target` at the gyroscope partition with `set_inflight_interval 1000`. Result: every completed request appends a compact summary line to both logs; the gyroscope log additionally collects periodic in-flight snapshots from the hidden `RequestFlight` sibling. The new `TopologyCompactSummaryTest` integration test loads each TSL in-process against a real CommandInterpreter + Router and asserts both partitions register, the Tee fans out to both, and the three `:config` verbs land on RequestBuilder.
...
- **`SchemaParityAuditTest` — M1 acceptance gate** (`#[Group('parity')]`). For every legacy SSE controller's payload shape, asserts every field is present in the new compact-summary / inflight-snapshot output. If a field drifts away from parity, this test fails LOUDLY — preventing the M5 deletion of `requests-stream-controller` and `gyroscope-stream-controller` from landing before the new feed catches up. The legacy field lists are frozen inline in the test as the audit authority.
