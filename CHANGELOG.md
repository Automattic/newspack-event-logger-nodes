...
- **`Request_Builder` `set_alerts_target` verb — `alert` entries dual-forward to the fleet-alert journal.** `alert` firehose entries now forward to BOTH the alerts journal (`alerts:partition` → `alerts.p0`, the same dir the substrate's `Alerts::emit()` writes) and the errors target; `emit_error` became `emit_entry` with an explicit target. Wired in `request-builder.tsl`.
