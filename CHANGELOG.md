...
- **A garbled discovery interval free-spun the drain.** `make_node Discovery_Collector discovery-collector abc` cast to 0 with no notice — as did `0` and `-1` — and a 0 ms timer takes an own Event_Framework slot that is due on every iteration, so the hub minted a signed `discovery.get` per spoke as fast as the loop turned. The interval token now goes through the substrate's `Timer_Node::parse_interval()`: digits or an `InvalidArgumentException`, floored at the substrate's shared `Timer_Node::MIN_INTERVAL_S` — this plugin's byte-identical private copy of that constant is gone, since the hitchhike boundary it names belongs to the base class. Requires newspack-nodes with both.
