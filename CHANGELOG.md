...
- **The Request Log and Gyroscope request-id link is no longer faint under decorative themes.** Its `$term-accent` used `color-mix(…, var(--np-primary), …)`, but `--np-primary` is undefined under non-Newspack hub themes (which define the universal `--cyan`), so the `color-mix` went invalid and the link fell back to a faint inherited gray. It now uses `var(--cyan, #003da5)` (universal accent + Cobalt floor), like `performance-dashboards` — readable on every theme.
