...
- Wrapping the dashboard UI strings for translation (`@wordpress/i18n` `__()`/`_n()`/`sprintf`, domain `newspack-event-logger-nodes`); the Tier-1 dashboards are done — `LogEntriesTable`, `Inflight`, `RequestStream`, `ErrorLog`, `HookSelectorModal`. `ConnectionBanner` consumers pass a translated `message`; `COLUMNS` label/tooltip values and category descriptions are wrapped, while object keys and comparison/map values stay raw.
