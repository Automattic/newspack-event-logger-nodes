...
- **No site filters `newspack_nodes/capability_map`.** Every site has the granular capabilities installed, so `read` resolves to `newspack_nodes_read` on all 41.
...
- **Every credential that reaches a stream belongs to a named account.** The hubs pull as a dedicated `newspack-nodes-hub` user, with one application password per spoke. The aggregator passwords the hubs used before that user existed, held by the shared `adminnewspack` administrator, were deleted on 2026-09-23 from all 27 sites that carried them. `adminnewspack`, which `allowed_users` admits, still holds unrelated passwords: two Pugpig integration tests on lookout.co and spacenews.com, a "Debug access control" password on spacenews.com, and a never-used "Cache Cozy" password on spacenews.com and Bangor. None of this is unauthenticated access, but each of those reads the raw stream and holds full administrator rights besides.
