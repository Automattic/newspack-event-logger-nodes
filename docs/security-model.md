...
**Code:** `includes/class-log-manager.php`: `URL_REDACT_PATTERN` (:146), `message()` (:1148); `Gyrobase/Log.pm`: `_redact_url` (:393), `_write_entry` (:1117); `includes/app/class-core.php`: `HOOK_ARG_KEEP` (:134), `shaped_argument()` (:325), `without_literals()` (:820); `newspack-pyrobase/includes/runtime/class-log.php`: `sql_shape()` (:534), `signature_shape()` (:574); `tools/check-firehose-parity.py`; `tools/survey-firehose-keys.php`.

`Log_Manager::message()` and `Gyrobase::Log::_write_entry` run `URL_REDACT_PATTERN` over every string message, and `check-firehose-parity.py` asserts all three properties: the two patterns are identical, each applies where the message is assembled, and neither touches the queued job body, which is what the hub runs the job from. [URL-secret redaction](architecture-guide.md#url-secret-redaction) covers the pattern's place in the write path.
...
- **URL query parameters.** A denylist of 25 names governs these: the censuses show 96 and 62 distinct parameters, most diagnostic (`page`, `action`, `rest_route`, `oid`, `film`, `sort`, `utm_*`), and the performance dashboard groups by URL, so an allowlist does not fit. The census surfaces `client`, `sig`, `signature` and `appid` carrying credential-shaped values, and one `user_email`; the pattern names `client`, `sig` and `signature`, and `appid` and `user_email` pass.

### The request id

**Code:** `includes/class-log-manager.php` (:962-968): `HTTP_X_A8C_REQUEST_ID`, else `UNIQUE_ID`, else a generated id, capped at 64 bytes.

The request id groups a request's firehose lines and comes from the `X-A8C-Request-Id` header when present. The model assumes WP Cloud overwrites that header at the edge; if it does not, a client can pick its own id and collide with or forge another request's.

### The Ask brief

**Code:** `includes/app/class-ask-assembler.php`: `entry_shape()` (:373), `for_request()` (:85, the entry map at :96-99), `ENV_ALLOWLIST` (:76); `includes/app/class-performance-ci-node.php`: `ask_request()` (:1311); `includes/app/class-mcp-controller.php` (:136-141); `src/overview/askBrief.js:186-188`, the `entry:` case (:276) and `askClaudeUrl()` (:35).

An Ask brief summarizes one request, URL or log entry for pasting into an AI assistant. Its `env` field passes a six-key allowlist (`method`, `request_method`, `status_code`, `worker_type`, `partition`, `server_name`); its `entries` field does not: `for_request()` copies up to `MAX_ENTRIES` (60) entries verbatim through `entry_shape()`, `environment_v3` among them, a map of 34 server variables including `REMOTE_ADDR`, `HTTP_USER_AGENT`, `HTTP_X_FORWARDED_FOR` and the JA3 and JA4 TLS fingerprints. The dashboard shows a request brief's entries as a count, so the map leaves by two other doors: the `performance_ask` MCP tool, to any agent holding a read-scoped session, and a single-entry brief, rendered in full (`askBrief.js:276`) and carried to claude.ai in the "Ask Claude" query string (`askClaudeUrl()`). [The environment allowlist](architecture-guide.md#the-environment-allowlist) covers what the writer admits into `environment_v3`.

## The remote-job rewrite

**Code:** `includes/class-remote-job-rewrite-node.php`; substrate `includes/class-job-worker-node.php` (:207-218); `newspack-pyrobase/includes/runtime/class-evtemplate.php`: `is_this_server()` (:291).
...
**Code:** `includes/app/class-mcp-controller.php` (:181-193); `mu-plugins/00-newspack-profiler.php` (:173-180); `includes/admin/class-admin.php`.
...
- **The request id trusts the `X-A8C-Request-Id` header.** Whether WP Cloud overwrites that header at the edge is open, and the answer decides whether a client can forge another request's id.
- **`entry_shape()` copies entries verbatim.** The `env` field passes `ENV_ALLOWLIST`; the `entries` field does not, so `environment_v3` leaves through the MCP tool and the single-entry brief. Whether `entry_shape()` should apply the assembler's allowlist is open.
...
- **Fail-soft stats, fail-closed SSE slots** ([decision 3](architecture-decisions.md#decision-3-fail-soft-stats-fail-closed-sse-slots); [Real-time path](architecture-guide.md#real-time-path-messagesstream--slot-pool)). A cache failure shows "no data" and refuses new streams with HTTP 429, because the slot pool is the rate limit.
