...
`read` is the widest role and its surface is the raw log, so how many accounts hold it is the fact that sizes every disclosure below. This was surveyed on 2026-09-23 across 42 sites: the community hub, its 23 spokes and the dev site; the `eln` hub and three test sites; four staging clones; eight production sites; and elsol.com.ar, an `eln` spoke that runs the substrate with the logger inactive.

- **No site filters `newspack_nodes/capability_map`.** Every site has the granular capabilities installed, so `read` resolves to `newspack_nodes_read` on all 42.
- **The capability admits between 1 and 50 accounts a site.** The install grants it to every role holding `manage_options`, so beside administrators, the community sites' `admin` role and the hub user it reaches Bangor's custom `audience_admin` role and, on fpinews.org, gazettenet.com and concordmonitor.com, the `contributor` and `revisor` roles, which hold `manage_options` there.
- **On Bangor the install has been lost twice, and what reverted it is not established.** Each time the role row lost `audience_admin`'s `newspack_nodes_manage` and the `newspack_nodes_hub` role, while the option that switches the capabilities on stayed set. On 2026-09-23 a first install was gone within four seconds and a second within about a minute; a third held for the eight minutes it was watched. Each install writes the row, and a fresh process reads the write back, so the loss is a later overwrite of the whole row. Nothing on Bangor filters writes to that option. PublishPress Capabilities, which manages Bangor's roles, hooks the row only as a read filter on its own admin page. gazettenet.com runs the same PublishPress release, and its install holds.
- **`allowed_users` narrows that to between 1 and 3 accounts a site.** The list names three operator accounts on 41 sites and one on the last, and the substrate applies it after the capability, at every door `read` opens.
...
Both stream routes, `messages/stream` and `log/stream`, answer only to `Capabilities::can( READ )`: the mapped capability through `current_user_can()`, then `allowed_users`, with no nonce, because a nonce would break the hub's pull. Checked on 2026-09-23 across the same 42 sites, all hosted on WordPress.com Atomic behind its edge, no path reaches either route without a WordPress login.

- **An anonymous request is refused on every site.** On 41, both routes return 401 `rest_forbidden` at the pretty path and through `?rest_route=`. The Atomic edge answers the last, a staging clone, with 480 for the whole site. In-process, an anonymous user fails the check on all 42, so no `user_has_cap` filter opens it.
...
- **Every credential that reaches a stream belongs to a named account.** The hubs pull as a dedicated `newspack-nodes-hub` user, with one application password per spoke. The aggregator passwords the hubs used before that user existed, held by the shared `adminnewspack` administrator, were deleted on 2026-09-23 from all 27 sites that carried them, as were the never-used passwords on spacenews.com, Bangor and elsol.com.ar. `adminnewspack`, which `allowed_users` admits, still holds unrelated passwords: two Pugpig integration-test passwords on each of lookout.co and spacenews.com, and a "Debug access control" password on spacenews.com. None of this is unauthenticated access, but each of those reads the raw stream and holds full administrator rights besides.
...
- **SQL.** Every quoted literal becomes `?`, comments are dropped, an `IN (…)` list collapses to one placeholder, and identifiers stay, since SQL has no keys to allowlist. This covers the `sql` span, any hook argument opening with a statement keyword, and the query signature, where `signature_shape()` blanks attribute values and keeps class and options. The statement keeps the layout it was asked with. Pyrobase's `Runtime\Log::sql_shape()` and `Gyrobase::Log::sql_shape` apply the same rules, and the three read one `sql-shape.json` case list that `check-firehose-parity.py` holds identical.
