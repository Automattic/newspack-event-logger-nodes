...
- **On Bangor the install does not stick.** `audience_admin` holds `newspack_nodes_read` and `newspack_nodes_tune` but not `newspack_nodes_manage`, and the `newspack_nodes_hub` role does not exist, while the option that switches the capabilities on is set. Running `wp nodes caps install` again has not held, and what reverts it is not established. Bangor manages its roles with PublishPress Capabilities; of the other 40 sites only gazettenet.com runs it, and gazettenet has not installed the granular capabilities.
