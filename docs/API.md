...
| `significant_events` | string[] | Hooks that get per-callback profiling and are exempt from auto-disable. `sql` and `http` name the `query` and `pre_http_request` filters their spans cover, and take effect under `log_queries` and `log_http`. |
...
| `log_http` | bool | Time every outbound HTTP request as a span, between `pre_http_request` and `http_api_debug`. Off unless set, as every diagnostic is. |
| `log_plugin_loads` | bool | Write the profiler mu-plugin's per-plugin load timings as spans. Off unless set. The mu-plugin measures either way; this decides whether the record carries two entries per site-activated plugin. |
...
| `trace_callers` | int | Deep caller chains recorded per request on a span's start entry as `caller`, budgeted per CALLER of each hook, query statement shape and outbound URL; 0 is off, and a stored `true` is a count of 1. |
...
| Verb | Role | Args | Returns |
|------|------|------|---------|
| `overview` | READ | `--server`, `--breakdown` (comma-separated), `--categories` | `{ total_requests, global_avg_ms, global_avg_peak_mb, aggregate_time_series, global_leaderboard }`, plus `breakdowns` keyed by dimension when `breakdown` is given and `category_time_series` when `categories` is. A `breakdowns` dimension is bucket key => dimension value => a positional `DIM_SUMS` triple, `[ count, sumMs, sumPeakMb ]` — requests, summed milliseconds, summed peak MB — never key names ([decision 18](architecture-decisions.md#decision-18-a-stored-value-may-be-positional-and-then-its-indexes-are-named-constants)). It is the STORED shape unchanged: the series has no storage/display boundary, so nothing translates it on the way out. The site totals come from the global `hourly` namespace, which has no server dimension; `server` scopes the leaderboard and the breakdowns only. A dimension outside `DIMENSIONS` throws `invalid breakdown dimension` rather than answering about the rest. `category_time_series` is `{ names, buckets }` — a name TABLE plus, per bucket, positional `[ nameIndex, t, c, n ]` rows. A category is one hook, callback or plugin, so its name would otherwise be spelled once per bucket, 288 times across a retention window: 838KB of an ~1.1MB reply. Nothing is capped or ranked — each bucket still holds every category the STORE kept for it — and `t` is rounded to four decimal places, a tenth of a nanosecond on a millisecond sum. |
| `urls` | READ | `--sort` (default `count`), `--order` (`desc`), `--limit` (50, clamped 1–1000), `--offset` (0, clamped 0–10000), `--search`, `--server`, `--errors_only`, `--include_workers` | `{ data, rows, totals, slowest, ranked, as_of, filters, limit, offset }` — the paginated, sortable URL leaderboard plus the totals and the slowest ten for whatever the filters left. An unknown `sort` falls back to `count`, an unknown `order` to `desc`. Worker traffic is excluded until `include_workers` opts it in. `errors_only` keeps the rows whose status buckets fail to account for every request — the timeouts and fatals that never reached a bucket at all — so a 5xx-heavy URL is excluded, a 5xx being a real response. Each kept row, and the totals, gain `errors`, that shortfall, beside a `count` that still counts the URL's traffic, so every mean and share divides what it summed; and a `count` sort ranks rows by `errors` instead. `search` matches a URL word or word prefix, not a substring — the term is lowercased, trimmed and split on every non-alphanumeric run, and each token of two characters or more (cut to twelve) must begin a word of the URL's path. The index answers a token of `URL_TOKEN_PREFIX_MIN` (3) characters or more, because a shorter prefix names most of a site and narrows nothing; a shorter token among several simply does not narrow, while the rest still do. A term the index cannot serve at all — no token, every token under three characters or saturated, or a set past `URL_SEARCH_MAX` (5,000) live candidates — is answered by folding the index instead, which matches the same words. A hash leaves a token's set once a retention window has passed without naming it. `rows` is the pager's count, every row the filters left including the folded `Other` and `Other:worker` overflow rows, while `totals.urls` counts the distinct URLs among them; an overflow row is one row standing for many URLs, so the two are never expected to agree. `totals` answers zeros for a scope that holds no rows; `filters` echoes what was applied, so a narrower number never reads as the site's. `ranked` is true when an unfiltered page whose `offset + limit` is inside `URL_RANK_N` (200) was answered from the writer's per-bucket ranked lists — its `avg_ms` and `avg_peak_mb` are then the mean of the per-BUCKET averages at each bucket's own tier, a five-minute bucket inside the fine tail and a folded hour behind it, every stored bucket weighing the same; an hour-tier entry therefore contributes one average covering that hour where a fine entry contributes one per five minutes, and nothing reweights them. Its `data` carries no overflow row, and a URL outside every bucket's top 200 is absent from it, while `rows`, `totals` and `slowest` still come from the fold and stay request-weighted. A ranked page also needs a list for every hour of the read plan behind the leading one, in every partition's store: those hours have no fine buckets left to read, so one missing list takes the page back to the fold. It needs a cached header too — the `rows`, `totals` and `slowest` no list can answer — so the first poll of each five-minute bucket folds that header and answers from the same fold, `ranked` false, and the polls after it read the lists. `as_of` is the Unix time the page was built, cached with it, which the dashboard measures each row's `last_updated` age from. |
| `dump_url` | READ | `hash` (required, positional, `[a-f0-9]{8,64}`), `--server`, `--breakdown`, `--categories`, `--since` | `{ stats, requests, scan_stopped_early, requests_window_start, aggregate_flame, aggregate_profiles, last_modified }`, plus `breakdown_time_series` and `category_time_series` when asked for — the former in the positional `DIM_SUMS` shape `overview`'s `breakdowns` carry, the latter in the same `{ names, buckets }` shape `overview` uses, so one encoder and one chart serve both. Without `--server`, it reads the one server `urlmap` names for the hash, and every server when the name has expired or that server holds no row. `--breakdown` refuses any dimension but the six a URL carries, `server` included, because a URL belongs to one server. Throws `URL not found` for an unknown hash and `invalid hash format` for a malformed one. |
...
Notable bounds, all [`Performance_CI_Node`](../includes/app/class-performance-ci-node.php) constants: `MAX_INDEX_SCAN_S` 10 seconds,
...
The stream's lifecycle — the `retry` and `connected` envelopes, heartbeats, `sse_idle_timeout`, the slot lease and its keepalive — is the substrate's, drawn in its [API](https://github.com/Automattic/newspack-nodes/blob/main/docs/API.md#sse-stream).
...
holds the three copies of `sql-shape.json`, the case list this plugin's `without_literals()`,
