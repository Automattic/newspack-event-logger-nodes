...
| `log_plugin_loads` | bool | Write the profiler mu-plugin's per-plugin load timings as spans. **Absent means ON**, as `log_http` is. The mu-plugin measures either way; this decides whether the record carries two entries per site-activated plugin. |
