...
| `request_detail` | `{ rid (required), partition?: int = 0 }` | Full request body + merged flame data. `partition` is a hint: searched first, then the rest, so any rid `request_search` locates resolves here too. Throws TM_ERROR `invalid partition` for out-of-range partition; `Request not found` for unknown rid; the `budget spent` message above when the walk ended first. Requires `manage_options`. |
| `ask` | `{ descriptor (required), server?, context? }` | The brief for one picker descriptor: `url:<hash>`, `request:<rid>[:<partition>]`, `span:<name>`, `entry:<n>` or `category:<name>`. A `span:` or `entry:` brief also needs its `request:` descriptor as context, outermost last; on `request:` the partition is a hint, not a filter. Every brief carries the measurement `caveat` and a `fetch` pointer naming the verb that returns the same subject in full. A `span:` brief answers for the parent whose copies of that name hold the most time, and carries `elsewhere` (`ms`, `count`, `parents`) for the copies under other parents — omitted when every copy sits under one parent. `server` scopes a `url:` brief the way `urls` scopes its rows. Requires `manage_options`. |
