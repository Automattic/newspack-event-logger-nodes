...
| `ask` | READ | `descriptor` (required, positional; further context descriptors follow it, outermost last), `--server`, `--context` | The brief for one picker descriptor. A `span:` or `category:` resolves in its container: under a `request:` from that request, under a `url:` from that URL's aggregate flame or profile as per-request means. |
