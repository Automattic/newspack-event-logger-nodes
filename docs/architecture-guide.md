...
| `urlsrv` | The URL index's server index, `urlsrv:{bucket}` -> `{ server_key => server_name }`: every server holding rows in the bucket, reader or worker. The keyspace cannot list itself, so this is what an unscoped read, the hour fold, the probe and the ranker's gap-fill enumerate. Capped at `Stats_Store::MAX_SERVER_VALUES` names: past that a new server's rows go to the `Other` server key (`Stats_Store::admit_servers()`), each path re-cut for it by `Stats_Store::refile_path()`, so it carries the whole URL, and a bucket's keys stay bounded whatever Host headers arrive. Mirrored in full, like `urls` | `min( min_lifetime, 7200 )` |
| `urlsrv_h` | The server index's COARSE tier, `urlsrv_h:{Y-m-d-H}`, folded by `roll_up_hours()` beside `urls_h`; past `MAX_SERVER_VALUES` servers the quietest fold into `Other`. DERIVED, not mirrored | `min_lifetime` |
