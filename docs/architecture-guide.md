...
| `url_dim` | per-URL dimensional series, keyed per bucket, every dimension in the value, each one a `DIM_SUMS` triple as in `dim` above | `min_lifetime` |
...
| `urlrank` | The writer's ranked top-N of one `urls` bucket: `urlrank:{sort}:{order}:{bucket}`, one list per `URL_SORTS` key and `URL_ORDERS` direction, fourteen site-wide lists a bucket. An entry is POSITIONAL ([decision 18](architecture-decisions.md#decision-18-a-stored-value-may-be-positional-and-then-its-indexes-are-named-constants)), indexed by `Stats_Store::RANK_*`: the hash, the row's thirteen scalars `ROW_COUNT` to `ROW_WORKER` and never the `ROW_SRV` split, which a server-scoped list has already projected; the `url` lists carry the path they ranked by at `RANK_PATH`. `URL_RANK_N` (200) entries, ranked over the bucket's WHOLE stored content — the merged reader-family shards each row intent landed, through the intent's `landed` closure, plus ONE multi-get of the shards that flush did not land — so a list describes the bucket rather than the flush, and the overflow and worker rows it holds out never rank. `Flame_Builder_Node::rank_flushed_buckets()` ranks per `flush_writes()` chunk, so a replay holds no more than a chunk's buckets. The two averages rank on the BUCKET's own average, `sum_ms / timed_count` and `sum_peak_mb / count`, so a page hit rarely but slowly ranks on how slow it is rather than on how often it is hit ([decision 28](architecture-decisions.md#decision-28-the-writer-ranks-each-bucket-and-indexes-each-name-and-the-page-reads-those-rather-than-the-index)). DERIVED from `urls`, so not mirrored. A refused list is one `print_less_often()` line naming the bucket, and nothing probes this tier: the bucket is ranked again only by a later flush that writes into it, and a ranked page missing one fine list is simply short that bucket's rows. A page folds only when no store held a list at all | `min( min_lifetime, 7200 )` |
| `urlrank_h` | The ranked lists' COARSE tier, `urlrank_h:{sort}:{order}:{Y-m-d-H}`, `URL_RANK_N_HOUR` (500) entries because one list ranks twelve buckets' folded rows. Written by the same `roll_up_hours()` pass that folds `urls_h`, from the rows it just folded. An hour holding rows and names but no lists is ranked from those stored coarse rows instead and never folded again: its fine buckets may be gone, so a second fold would overwrite the hour with nothing. A separate DONE marker, `urlrank_h:done:{Y-m-d-H}`, rides the ranking's own batch whatever its lists answer, and it is the one key `url_hours_derived()` probes, so no re-probe ranks a refused hour again; `ranked_page()` serves an hour only where the list it pages is present, and folds otherwise. A late write that lands in a folded hour's key forgets its DONE marker, so the probe finds the hour stale and `rank_hours_from_store()` re-ranks it from the store; a refused list is logged and not retried | `min_lifetime` |
| `urlrank_s` / `urlrank_sh` | One server's lists, `urlrank_s:{server_key}:{sort}:{order}:{bucket}` and `urlrank_sh:{server_key}:{sort}:{order}:{Y-m-d-H}`, one set per server a row's split names. The server scope rides in the KEY, the form [decision 14](architecture-decisions.md#decision-14-a-server-scope-the-key-cannot-carry-rides-inside-the-value-and-is-applied-as-a-projection) rejected for the stored rows, and that rejection stands: `urls` gained no mirror bound and is still mirrored in full at `MAX_URLS_PER_SHARD` rows a shard. What pays for the key form here is that a list is `URL_RANK_N` entries whatever the rows behind it, and that all four rank tiers answer `Stats_Store::is_derived()`, so the mirror holds none of them. An entry is the row projected to that server through `Stats_Store::url_row_scoped()`, the writer's twin of the reader's `swap_url_server_sums()`, so the two paths cannot disagree about a scoped row. Same shape and same TTLs as the two above | `min( min_lifetime, 7200 )` / `min_lifetime` |
| `urltoken` | The search index, `urltoken:{token}` -> `{ hash => the unix seconds that URL was last named }`, filed under every prefix of every word of the path from `URL_TOKEN_PREFIX_MIN` (3) characters to `URL_TOKEN_PREFIX_MAX` (12), the path lowercased and split on every non-alphanumeric run. A two-character term token is unserved and sends the term to the fold: a prefix that short names most of a real site and would narrow nothing. `merge_token_set()` restamps this flush's hashes and saturates when the count passes `URL_SEARCH_MAX` (5,000); before it counts, it drops every entry a retention window has passed over, whenever the set holds one. The walk is DECIDED before it runs — one pass over the stamps that stops at the first expired one, skipped altogether where the flush would pass the cap anyway — because a set of four re-read entry by entry on every flush is the cost that decision exists to bound. The `TOKEN_SATURATED` sentinel (`*`) is an entry like any other, under its own stamp. So a set SHRINKS — a hash leaves it when its URL stops being named, not when the key dies — and a live sentinel is returned unchanged, so `flush_writes()` skips the write, the key keeps the TTL it had, and the token rebuilds live-only when that expires. `url_token_sets()` hands a reader the hashes, and `false` for a token no read can answer — one below `URL_TOKEN_PREFIX_MIN`, refused without a read, or one that has saturated; the stamps are the writer's alone. Written as read-merge-write intents in the flush's own batch, for the names `persist_url_names()` wrote, so a URL costs `sum( min( word, 12 ) - 2 )` keys over its words of three characters or more — six for `/wombat-7731` — once per URL per half retention window; a refused write is logged and not retried, and its URLs are filed again at that refresh. Not bucketed, so never open. DERIVED, and not mirrored | `min_lifetime` |

![The memcache key layout: an install scope the substrate's salt lives in, then evlog, the partition p{N}, one of nineteen namespaces, the scope parts and the bucket LAST; a table of every namespace's key shape, value and TTL, the four urlrank tiers ranking each bucket's top 200 or 500 rows per sort key and direction and urltoken filing each URL name's token prefixes; a timeline of what one URL-index read asks for at the default 12-hour retention, eleven coarse urls_h hour keys behind thirteen fine urls buckets and the rest of their hour; and the URL row's fourteen positional indexes, the eight that add, the two extremes, and the per-server srv split.](img/ag-memcache-keys.png)
