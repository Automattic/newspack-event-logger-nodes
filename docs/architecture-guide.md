...
- `k` — category. The ` (start)` / ` (complete)` suffix is what pairs a span, and four span shapes share the field with the one-shot entries:
  ...
  - A **listener span**, `"<callable> @<priority> (start)"` / `"<callable> @<priority> (complete)"`, which `App\Core::wrap_callbacks()` mints around every callback on a hook the governing rule names a significant event. The callable is `short_name()`'s shortened form — `Class::method`, or `{closure}:file.php:12`. `Flame_Tree::is_listener_span()` is the seam a reader classifies one with, beside the other span classifiers, over the private `LISTENER_PATTERN` (`/ @-?\d+$/`), and the `-` is load-bearing: a callback registered at a negative priority reads as a custom event without it. [`Findings`](../includes/app/class-findings.php) runs every span through that seam, which is what stops a listener being labelled `custom`.
  ...
  - Anything else is a **one-shot entry**: `request`, `worker_type`, `worker_partition`, `environment_v3`, `resources`, `memory`, `message`, `job`, `error`, `warning`, `info`, `alert`, `stderr`, or a custom event. `message` carries the FROM, ID and KEY of the record that opened a job context, so a reader can seek back onto the log that enqueued it.
...
A worker request names itself as two entries of its own, `worker_type` and `worker_partition`, written by `log_worker()` right after `process (start)`. The type is `NEWSPACK_NODES_WORKER_TYPE` where the substrate set it in the serving process — a job context, the reconcile pass — and otherwise the name `Log_Manager::PLATFORM_WORKERS` gives the request path: `cron` for the `/wp-cron.php` loopback, and `restapi` for the substrate's `/wp-json/newspack-nodes/v1/{command,log/stream,messages/stream,workers/spawn}` endpoints, one name because the path already names each, whose env var is set only at REST dispatch, after the request was opened. The partition entry follows the type only where `NEWSPACK_NODES_WORKER_PARTITION` is set. Neither var is in `ENV_ALLOWLIST`, so the `environment_v3` entry never carries them, and the shipped seed skips none of these URLs: a rule that logs them keeps them on the worker rows. [`Request_Builder_Node`](../includes/class-request-builder-node.php) reads the `worker_type` entry, sets `is_worker`, and appends the worker type to the request URL through `resolved_request_url()` — `?<worker_type>` where the URL carries no query yet, `&<worker_type>` where it does — so worker traffic gets its own URL rows instead of polluting the real ones. Without that exclusion, the fleet's spawn cycle would dominate every leaderboard. [`Request_Flight_Node`](../includes/class-request-flight-node.php) resolves its in-flight gyroscope rows through that same method, and the two must agree: a job's execution and the request that enqueued it share one `/jobs/{handler}/{id}` URI, so a second spelling collapses them onto one URL row and undoes the split silently.
...
Truncation is silent apart from the `truncated` flag and never throws, so size discipline is the caller's responsibility. The firehose is appended by every request process with no lock, so a firehose entry has no escape: shape a large value at the producer or split it across entries. A partition with one writer can lift its cap to 32 MiB — by a held lock through `allow_large_writes()`, which is how [`\Newspack_Nodes\Job_Intake::queue()`](https://github.com/Automattic/newspack-nodes/blob/v2.56.0/includes/class-job-intake.php) admits a large job, or by the `void_warranty` assertion the `requests` and `flames` partitions carry.

`tools/check-firehose-parity.py` does not compare the PHP and Perl truncation paths at all: it holds `ENV_ALLOWLIST` membership and order, `ENV_VALUE_MAX`, the elision marker and the URL-redaction pattern equal, holds both producers to applying that pattern where the message is assembled and to leaving a queued job body alone, and holds the three `sql-shape.json` copies identical.
...
![Rule matching and storage: three ranks, exact path plus query prefix over exact path over path prefix, with length breaking ties only within a rank; a worked walk of /wp-json/newspack-nodes/v1/command?x=1 down a sorted ruleset of exact skips an operator wrote, where the exact skip governs and the /wp-json and / log rules sit below every exact rule; and the four storage tiers, the autoloaded rule list, a heavy rule's non-autoloaded hooks option past INLINE_HOOK_LIMIT 100, its Table mirror eln-rule-hooks at TTL 3600, and hydrate_array() for the wire.](img/ag-rule-matching.png)

Patterns are written as site paths, so a caller holding an absolute URL passes its path. The shipped default, [`Settings_Schema`](../includes/class-settings-schema.php)'s `rules`, which `Rule_Set::seed_from_config()` applies until the editor writes the option, is the `/` LOG rule alone; `/wp-json/newspack-nodes/v1/{command,log/stream,messages/stream,workers/spawn}` and `/wp-cron.php` need no skip, because `Log_Manager` names each as a worker.
...
![One five-minute bucket on four tiers: requests file into $pending by the bucket they finished in; flush() sums them into memcache at most every five seconds; every landed write refreshes a held frame in RAM, carried in the offsetlog under a 16 MiB cap until the bucket closes; flame-stats.pN receives the frame once, at the first checkpoint after the close, and keeps it for twice the stats window. Four cards cover what is mirrored and the 10,000-frame backstop, a respawn and the 32 MiB cliff behind its carry, the read-back through locate_by(), and why the mirror carries no version of its own.](img/ag-stats-mirror.png)

Reading it back is BUDGETED, and only for the dashboard. [`Partition_Node::locate_by()`](https://github.com/Automattic/newspack-nodes/blob/v2.56.0/includes/class-partition-node.php) cannot stop early on a key that is absent from the index, so every batch that misses on memcache costs a full pass over the mirror's `.idx` — ~192ms over 150,000 lines — and one cold `urls` poll issues thousands of them, sixteen shards by four partitions by twenty-five bucket chunks. So [`Performance_CI_Node::dispatch()`](../includes/app/class-performance-ci-node.php) starts each VERB with a fresh `stats_mirror_read_budget_ms` (default 5000, `0` off), `Flame_Builder_Node::arm_stats_reader()` stops consulting the mirror once it is spent, and the reader answers from memcache alone for the rest of that verb — one poll batches `overview` and `urls` into a single POST, so a response can spend the budget twice — the "no data" degradation the stats readers already take. Naming the rows a verb shows is an answer of its own: `resolve_urls()` runs its one `urlmap` read per partition under `Flame_Builder_Node::with_own_mirror_read_budget()`, a fresh budget that resumes the verb's accounting afterwards, because the names resolve last, after the walk that spends the verb's, and a page of counts against blank URLs is no page. An absence the walk answered is remembered, so the budget is spent learning it once rather than on every poll: `arm_stats_reader()` also sets the store's `$absence` seam to `Stats_Store::absence_holds()`, and the Table stores a marker for each key the mirror did not return, held for what is left of the window for a bucket that closed and for `ABSENCE_HOLD_SECONDS` for the open bucket or a key that is no bucket. Without it a sparse server, one with buckets the mirror holds no frame for in every window, spent the whole budget on the leaderboard's absent keys on every poll and its per-server series never filled in. `rehydrate_seam()` also skips any namespace `Stats_Store::is_derived()` names, since a read of one can only ever walk and find nothing. The WORKER's own seam, armed by `set_stats_target`, is unbudgeted: it is restoring its own state, not answering a poll.

`set_is_hub <eln:is_hub>` turns on the three per-server AGGREGATES — the `lb_s` leaderboard, the per-server dimensional series and the per-server category series — because only an aggregating hub has more than one reporting server to spread across them. The URL index's per-server keys are deliberately NOT gated by it: every URL row is filed under its server's key on a spoke as on a hub ([decision 30](architecture-decisions.md#decision-30-every-memcache-value-is-one-servers-carries-nothing-its-key-implies-and-fits-one-item)), so a spoke's table reads the same keys a hub's does.
...
class Flame_Builder_Node extends Node {
    ...
    private const MAX_CHECKPOINT_MIRROR_BYTES = 16777216; // held-frame carry budget
...
On flush, `Flame_Builder_Node` turns each pending bucket's accumulators into write intents — a key's parts, its bucket, and a merge closure that adds this worker's sums to whatever is stored and re-applies the namespace's cap where the schema sets one — and hands them to `flush_writes()`. That method chunks the intents at `WRITE_BATCH_KEYS` (500), reads a chunk with one `Stats_Store::bucket_get_multi()`, applies each intent's closure, writes the chunk back with one `bucket_set_multi()`, and calls an intent's `refused` callback for a key that did not land. A chunk whose read failed, which `bucket_get_multi()` reports through its `$failed` argument, writes nothing and logs once a window: its deltas are dropped ([decision 3](architecture-decisions.md#decision-3-stats-fail-soft)). Batching the writes is the same one-round-trip-per-batch discipline [decision 6](architecture-decisions.md#decision-6-get_multi-batching-is-essential) asks of the read paths. `flush()` then drops the accumulators it wrote; whatever has accumulated since rides `save_state()`'s checkpoint frame beside the read cursor, so a respawned worker resumes a half-accumulated bucket rather than losing it. Merging by addition is what lets several partitions write the same bucket without coordination; retention is the key's own TTL, so nothing prunes buckets by hand. A write into an hour this process has already folded goes to the coarse hour key instead of a fine bucket nothing would ever read again.
...
|---|---|---|
| `Flame_Tree` | [`includes/class-flame-tree.php`](../includes/class-flame-tree.php) | The pure flame-graph algorithm: stateless tree construction from one request's entries (LIFO span matching), duplicate-sibling numbering so aggregation does not collapse them, incremental sums-not-means merging across requests, and a finalize pass converting sums to averages and normalizing parent ≥ children. `MAX_STACK_DEPTH` and `MAX_RECURSION_DEPTH` are both 50; an aggregate child unseen for `AGGREGATE_EXPIRY_SEC` (3600) is dropped. Each span carries `t`, its start in ms from the request's own, which the browser's `withTimeSpacers` pass turns into a position on the x-axis, and `i`, the position in the record of the entry that opened it, which a Cmd-click on the frame hands the log table to find its row by |
| `Flame_Fold` | [`includes/class-flame-fold.php`](../includes/class-flame-fold.php) | The merging, RESUMABLE variant of that stack machine — static over a plain-array state, because an in-flight envelope rides the Consumer's checkpoint frame and an object would not come back. Same-name siblings under a parent collapse into one node with `count` / summed `value` / `max`, so a folded envelope costs O(distinct paths) rather than O(messages). A collapsed node ships `merged`, which is what stops the browser laying it out from a `t` that belongs to only the earliest of the spans inside it. A merged `sql` or `http` node also keeps `shapes`, the distinct statements or URLs it ran with a count and summed ms each ([decision 26](architecture-decisions.md#decision-26-a-merged-transport-node-keeps-the-statements-it-ran)), bounded per shape, per node and per record |
| `Findings` | [`includes/app/class-findings.php`](../includes/app/class-findings.php) | What is wrong with one stored record, COMPUTED rather than inferred: a fatal, a stamped rule this ruleset does not hold (`unresolved_rule`, which also skips the cold start, rides its stamp on every other finding of that record and strips their proposals), "insufficient instrumentation" as a first-class finding, unattributed time (`UNATTRIBUTED_SHARE` 0.5), dominant span (`DOMINANT_SHARE` 0.6, naming the outermost dominating span that ran more than once, since same-name siblings count as one repeat and a query holding 80% of a request because the content around it rendered ten times is ten renders to explain, and never the request's own `process` frame, which holds all of every request), plugin load (`PLUGIN_LOAD_SHARE` 0.25: the loads the profiler drop-in times, summed, naming the heaviest three, for the request where together they matter though no one of them dominates), repetition (`REPETITION_COUNT` 50), entry gap (`GAP_MS` 250) and truncation, returned worst-first. The fatal and the unresolved rule are the two findings that need no arithmetic and carry no proposal — PHP knew the message, file, line and offending plugin at the moment it stopped, and no rule edit fixes a crash or reaches a rule this ruleset does not hold. `MIN_DURATION_MS` (50.0) floors both share-based tests, which is the whole explanation for a fast request answering with nothing: 90% of 3ms is not a finding. Every finding names where it was measured and what rule edit would act on it, and the instrumentation proposal names that edit concretely — `LIFECYCLE_BRACKET`, the six hooks `plugins_loaded`, `init`, `wp_loaded`, `template_redirect`, `wp_head` and `shutdown` — a coarse bisect whose next round subdivides only the phase that held the time, which is what keeps a proposal from being forty hooks. A proposal's `direction` is as often `more` as `less`. `for_url()` is the cold start rather than a scaled-down `for_request()`: handed a URL's aggregate stats it runs none of the eight record-level detectors, answering `[]` when the governing rule already registers hooks and otherwise the single `insufficient_instrumentation` finding, whose proposal is the lifecycle bracket or a rule to create. A plain class, not a Node: a pure function of a record, so the picker and an agent produce identical evidence |
| `Ask_Assembler` | [`includes/app/class-ask-assembler.php`](../includes/app/class-ask-assembler.php) | The `?` picker's descriptor vocabulary — `overview:`, `url:`, `request:`, `span:`, `entry:`, `category:` — shaped into a brief, bounded by `MAX_ENTRIES` 60, `NEIGHBOURS` 2, `WORST_REQUESTS` 5 and `TOP_SPANS` 6. Shaping only; loading belongs to `Performance_CI_Node`. URLs go through `Log_Manager::redact_url()`, the environment is allowlisted rather than filtered, and `Findings::caveat()` rides every brief |
...
| `lb` | 5-min global leaderboard buckets, sums-not-means. Capped at `MAX_LB_CATEGORIES` (200) categories, the slowest by `sum_time` first, and lower when their estimated bytes pass `ITEM_BUDGET`; the tail folds into `Other`, and each category's entries trim at 100 down to 50 | `min_lifetime` |
| `lb_h` | the GLOBAL leaderboard's coarse tier, `lb_h:{Y-m-d-H}`, folded by the same `roll_up_hours()` pass that folds `urls_h` and holding the same shape, under the same caps, a fine bucket holds, so ONE `build_leaderboard()` fold serves both tiers. This is the heaviest read the dashboard makes — a category per hook, callback and plugin the site fires, 1,198 of them on a production hub, each with its own entry map, across 288 buckets and four partitions — so it takes [decision 17](architecture-decisions.md#decision-17-the-url-index-is-stored-at-two-resolutions-and-the-coarse-one-is-derived)'s two resolutions for [decision 17](architecture-decisions.md#decision-17-the-url-index-is-stored-at-two-resolutions-and-the-coarse-one-is-derived)'s reason. An hour the coarse tier cannot answer for has not been folded yet, and only the GRACE hour — the one immediately behind the fine tail — is answered from its fine buckets. A late record into a folded hour reaches both its fine `lb` bucket and this key while the key exists, the URL index's rule ([decision 19](architecture-decisions.md#decision-19-a-request-is-filed-in-the-bucket-it-finished-in-and-a-write-into-a-folded-hour-reaches-its-fine-bucket-and-the-hour-key)); a key its read misses is forgotten, and `url_hours_derived()` counts it among an hour's keys, so the reprobe re-folds it | `min_lifetime` |
| `lb_s` | per-server leaderboard, keyed by server, under `lb`'s caps. NO coarse tier: the servers present in an hour cannot be enumerated from the keyspace, and nothing indexes them for the leaderboard, so a server-scoped board still walks the window | `min_lifetime` |
| `urls` | 5-min URL index, one SERVER's rows per key and SHARDED, `urls:{server_key}:{shard}:{bucket}` ([decision 30](architecture-decisions.md#decision-30-every-memcache-value-is-one-servers-carries-nothing-its-key-implies-and-fits-one-item)), by the first hex digit of the url_hash — and by POPULATION, a `w` prefix on the token (`urls:{server_key}:w3:...`) naming worker traffic, which the table excludes by default. `server_key` is `Stats_Store::server_key()`, an FNV hash of the name, and `urlsrv` lists the servers a bucket holds rows for. Held only for `Stats_Store::FINE_TTL_SECONDS` (2h), against `min_lifetime` for the coarse tier that outlives it: the read plan asks for `FINE_BUCKETS` (13) plus the rest of their hour — just under two at the worst minute — and `urls_h` answers for everything behind that. The tier has exactly two consumers, `RECENT_BUCKETS`' last-hour rate and the fold, and two hours covers both; at that width it costs 24 buckets a shard, as many keys as the coarse tier holds at a 24-hour retention and twice its twelve at the default. Keyed by url_hash to a POSITIONAL row indexed by `Stats_Store::ROW_*`. `serialize()` writes every key NAME into every row, so the shape `Message` uses is the shape this takes: measured on live rows at 672 bytes a row named against 398 positional. The eight fields that ADD occupy indexes 0-7 in `ROW_SUMS` order, so one map describes the row's summed half. `ROW_FIELD_NAMES` is the one index-to-name map; `Performance_CI_Node::fold_index_row()` is the one place a stored row becomes the named display row. Index 13, `ROW_PATH`, is the row's one string: the path with no scheme or host for an https URL under the key's own server, the whole URL otherwise, cut to `MAX_PATH_BYTES` (1,024) with a trailing `…` (`Stats_Store::row_path()`). A shard is capped by its ESTIMATED bytes, `Stats_Store::overhead( 'url_row' )` plus each row's path length against `ITEM_BUDGET` (900,000), busiest rows first, and its tail folds into the two overflow rows below | `min( min_lifetime, 7200 )` |
| `urlmap` | URL name table, `urlmap:{hash}` -> `[ server_name, path ]`: the server the hash's rows are filed under, and the path `Stats_Store::row_path()` cuts against it, no scheme or host for an https URL whose host is that server, else the whole URL. Written once per URL per server it is filed under, and re-written only once half its own TTL has passed. `Stats_Store::get_url_names()` is the one join, `https://{server}{path}` for a path starting with `/` under a server that names a host, else the path as written; `Performance_CI_Node::resolve_urls()` is the one place a row gets its DISPLAY name through it. It is also the locator: an unscoped `dump_url` or `ask url:` reads the named server's keys alone, and every server's when the name has expired or its server holds no row | `min_lifetime` |
| `urlsrv` | The URL index's server index, `urlsrv:{bucket}` -> `{ server_key => [ server_name, shards ] }`: every server holding rows in the bucket, reader or worker, beside the int bitmask of the shards it wrote there, positional under `Stats_Store::SRV_NAME` and `SRV_SHARDS`. The keyspace cannot list itself, so this is what every read, scoped or not, the hour fold, the probe and the ranker's gap-fill enumerate, and each asks only for the shards an entry names: a key nobody wrote costs the mirror a full walk to say so. Capped at `Stats_Store::MAX_SERVER_VALUES` names: past that a new server's rows go to the `Other` server key (`Stats_Store::admit_servers()`), each path re-cut for it by `Stats_Store::refile_path()`, so it carries the whole URL, and a bucket's keys stay bounded whatever Host headers arrive. Mirrored in full, like `urls` | `min( min_lifetime, 7200 )` |
| `urlsrv_h` | The server index's COARSE tier, `urlsrv_h:{Y-m-d-H}`, in the same shape, folded by `roll_up_hours()` beside `urls_h`; each server it keeps is named with all 32 shards, because the fold writes every one, empty or not. Past `MAX_SERVER_VALUES` servers the quietest fold into `Other`. DERIVED, not mirrored | `min_lifetime` |
| `url` | per-URL flame/profile blob, `url:{hash}`. Its profiles take `lb`'s cap at half of `ITEM_BUDGET`, and each of its two flame-tree copies is pruned lightest leaf first to a quarter (`Flame_Tree::prune_lightest()`), in the stored copy alone | `max(3600, min_lifetime/24)` |
| `dim` | dimensional time series, keyed per bucket, one namespace per dimension and `$server` scopes it (`Stats_Store::dim_parts()`). Each value in a bucket is a POSITIONAL `DIM_SUMS` triple ([decision 18](architecture-decisions.md#decision-18-a-stored-value-may-be-positional-and-then-its-indexes-are-named-constants)), indexed by `Stats_Store::DIM_COUNT`, `DIM_SUM_MS` and `DIM_SUM_PEAK_MB` — requests, summed milliseconds, summed peak MB. It stays positional to the wire, so the two React sites that read it index it directly | `min_lifetime` |
| `url_dim` | per-URL dimensional series, keyed per bucket, every dimension but `server` in the value, each one a `DIM_SUMS` triple as in `dim` above. A URL belongs to one server, so that axis would hold one value; `dump_url` and `url_breakdown` refuse it | `min_lifetime` |
| `categories` | category time series, keyed per bucket, `$server` scopes it (`Stats_Store::cat_parts()`). Each category in a bucket is a POSITIONAL `CAT_SUMS` triple ([decision 18](architecture-decisions.md#decision-18-a-stored-value-may-be-positional-and-then-its-indexes-are-named-constants)), indexed by `Stats_Store::CAT_MS`, `CAT_CALLS` and `CAT_REQUESTS` — milliseconds of wall time rounded to `CAT_MS_DECIMALS` places, events fired, requests the category appeared in — which indexes differently from the `DIM_SUMS` triple of the `dim` namespaces beside it, where index 0 counts requests rather than events and time is index 1 rather than 0. The bucket also carries a synthetic `total` category holding the request's own wall time, which every ranking has to hold out — `Flame_Builder_Node::cap_bucket()` does it before the cap and `CategoryTimeChart` before the palette, since as a band it swamps every real category | `min_lifetime` |
| `url_cat` | per-URL category series, keyed per bucket, in the same positional `CAT_SUMS` shape | `min_lifetime` |
| `eln-urls-page` | Two entries, both read THROUGH `Table_Node::backed_by()`. One `urls` page per filter set, keyed by an md5 of every filter plus the window bucket, the retention, the store count and the page's field list, so every tab polling the same page shares one build a minute; and one `header` entry per scope, holding the `rows`, `totals` and `slowest` a ranked page takes from the fold, keyed by the scope, the window bucket, the retention, the store count and its own field list, and held `Stats_Store::BUCKET_SECONDS` — the bucket's own life — so the thirty-second fold runs once per five minutes while the ranked lists answer the table every poll. The header's miss folds THAT page and answers with it, so the poll paying for the walk is not also a ranked read. Its own Table outside the `evlog:p{N}` grammar, like `eln-rule-hooks`: a page folds every partition. A page over `URLS_PAGE_CACHE_MAX_ROWS` (250) rows is never stored, and a build the mirror read budget cut short is served with `ttl => 0` | 60 s, and 300 s for a header |
| `urls_h` | the URL index's COARSE tier, `urls_h:{server_key}:{shard}:{Y-m-d-H}` — one server's hour of merged rows in the same positional shape and under the same byte cap a `urls` bucket holds. DERIVED: `Flame_Builder_Node::roll_up_hours()` folds a closed hour once. Only the GRACE hour, the newest the read plan takes coarse, is answered from its twelve `urls` buckets when its key is missing; an older missing hour reads as empty. Not mirrored, for the same reason | `min_lifetime` |
| `urlrank_s` / `urlrank_sh` | The writer's ranked top-N of one server's `urls` bucket and hour: `urlrank_s:{server_key}:{sort}:{order}:{bucket}` and `urlrank_sh:{server_key}:{sort}:{order}:{Y-m-d-H}`, one list per `URL_SORTS` key and `URL_ORDERS` direction, fourteen for every server the key's `urlsrv` index names, empty where the server holds nothing rankable. An entry is POSITIONAL ([decision 18](architecture-decisions.md#decision-18-a-stored-value-may-be-positional-and-then-its-indexes-are-named-constants)), indexed by `Stats_Store::RANK_*`: the hash, the row without its path, and on the `url` lists the path at `RANK_PATH`. `URL_RANK_N` (200) entries a bucket, `URL_RANK_N_HOUR` (500) an hour; a tie breaks by hash. No list spans servers: the site's page is `Stats_Store::url_rank_window()` merging every named server's list per key and cutting it where one list over all their rows would. That is exact while no hour names more than `MAX_SERVER_VALUES` servers, because URLs are then disjoint by server; past it, a server admitted by name in one bucket and under `Other` in another can rank short. A site hour answers only when every server its index names has its list; a fine bucket answers with the lists it holds. Each server's hour carries a DONE marker, `urlrank_sh:done:{server_key}:{Y-m-d-H}`, written in the ranking's batch whatever its lists answer. `url_hours_derived()` names every indexed server whose marker is missing, and `rank_hours_from_store()` re-ranks the hour from its stored rows; a late write that lands in a folded hour forgets its server's marker. DERIVED from `urls`, so not mirrored; a refused list is logged and not retried | `min( min_lifetime, 7200 )` / `min_lifetime` |
| `urltoken` | The search index, `urltoken:{server_key}:{token}` -> `{ hash => the unix seconds that URL was last named }`, one set per server a URL's rows are filed under (`Other` past the index cap), filed under every prefix of every word of the path from `URL_TOKEN_PREFIX_MIN` (3) characters to `URL_TOKEN_PREFIX_MAX` (12), the path lowercased and split on every non-alphanumeric run. A two-character term token is unserved and sends the term to the fold: a prefix that short names most of a real site and would narrow nothing. `merge_token_set()` restamps this flush's hashes and saturates when the count passes `URL_SEARCH_MAX` (5,000); before it counts, it drops every entry a retention window has passed over, whenever the set holds one. The walk is DECIDED before it runs — one pass over the stamps that stops at the first expired one, skipped altogether where the flush would pass the cap anyway — because a set of four re-read entry by entry on every flush is the cost that decision exists to bound. The `TOKEN_SATURATED` sentinel (`*`) is an entry like any other, under its own stamp. So a set SHRINKS — a hash leaves it when its URL stops being named, not when the key dies — and a live sentinel is returned unchanged, so `flush_writes()` skips the write, the key keeps the TTL it had, and the token rebuilds live-only when that expires. `url_token_sets()` hands a reader the hashes of the servers it names — a scoped search its server's, a site search every server an index across the plan names — and `false` for a token no read can answer — one below `URL_TOKEN_PREFIX_MIN`, refused without a read, or one that has saturated; the stamps are the writer's alone. Written as read-merge-write intents in the flush's own batch, for the names `persist_url_names()` wrote, so a URL costs `sum( min( word, 12 ) - 2 )` keys over its words of three characters or more — six for `/wombat-7731` — once per URL per half retention window; a refused write is logged and not retried, and its URLs are filed again at that refresh. Not bucketed, so never open. DERIVED, and not mirrored | `min_lifetime` |

![The memcache key layout: an install scope the substrate's salt lives in, then evlog, the partition p{N}, one of seventeen namespaces, the scope parts and the bucket LAST; a table of every namespace's key shape, value and TTL, the server index urlsrv, the two per-server urlrank tiers ranking each server's top 200 or 500 rows per bucket or hour per sort key and direction, and urltoken filing each URL's token prefixes under its server; a timeline of what one URL-index read asks for at the default 12-hour retention, eleven coarse urls_h hour keys behind thirteen fine urls buckets and the rest of their hour; and the URL row's fourteen positional indexes, the eight that add, the extremes, the flags and the path.](img/ag-memcache-keys.png)
...
**Every value is capped to `Stats_Store::ITEM_BUDGET` (900,000 bytes) before it is written** ([decision 30](architecture-decisions.md#decision-30-every-memcache-value-is-one-servers-carries-nothing-its-key-implies-and-fits-one-item)): memcached's 1,048,576-byte item limit less a margin, uncompressed, under an estimate `Stats_Store::overhead()` makes for the configured serializer. The count caps:
...
- `MAX_DIM_VALUES = 20` — the `server` axis takes `MAX_SERVER_VALUES = 128`
  ...
  of a 24-spoke fleet's own picker. The URL index's server index, `urlsrv`, takes the
  same 128. See architecture [decision 30](architecture-decisions.md#decision-30-every-memcache-value-is-one-servers-carries-nothing-its-key-implies-and-fits-one-item) for when to raise it
- `MAX_URL_DIM_VALUES = 10`
- `MAX_CAT_VALUES = 50`
- `MAX_LB_CATEGORIES = 200`, and lower where the categories' estimated bytes bind
- A URL shard has no row cap: `cap_url_rows()` keeps the busiest rows its byte estimate
  admits, one server's rows per key

A URL belongs to one server, so per-server URL data is keyed by server rather than split inside a row, and `url_dim` carries no `server` axis: `dump_url --breakdown` and `url_breakdown` refuse it.

Overflow rolls into a synthetic `Other` value rather than dropping, so a total summed from a capped namespace is still exact. The URL index folds each shard's byte-capped tail the same way, into TWO overflow rows (`Stats_Store::other_key()`): `Other` and `Other:worker`, one per population, so the two shard families never fold their tails into each other. `Flame_Builder_Node::cap_bucket()` holds the `total` pseudo-category out of the ranking sort and puts it back afterwards, so a wide bucket can never fold the total into `Other`.
...
| `stats_mirror_read_budget_ms` | int | `ui:false` | `5000` | Milliseconds ONE dashboard verb may spend reading that mirror. `locate_by()` has no early stop for an absent key, so each batch that misses costs a full pass over the mirror's index; past the budget the verb answers from memcache alone, and an absence the walk answered is remembered so it is paid once, not per poll. `0` turns the reader's mirror read off. The worker's own read is never budgeted |
...
| `ask` | READ | The brief for one descriptor — `overview:`, `url:`, `request:`, `span:`, `entry:` or `category:` |
...
**The URL detail modal.** [`UrlDetailView`](../src/overview/components/UrlDetailView.js) draws the URL's whole story in six sections — the breakdown panel, the three category charts, the response-time scatter, the aggregate flame, the averaged profile breakdown and the recent-requests table — and it owns less of that than the list suggests. Sorting lives upstream: `PerformanceDashboard` sorts and hands back `sortedRequests`, and `requestSort` only tells the headers which arrow to draw. The "Errors Only" toggle lives upstream too: `PerformanceDashboard` holds it, so it survives the modal unmounting for a request and back, and sets it from the table's own filter whenever a URL opens, so a URL picked under "Errors Only" opens narrowed. It narrows the row list, the "Recent Requests (%d)" count and the bar-scaling maximum together. The requests table virtualizes against the ancestor `.components-modal__content`, so the view must be mounted inside a WordPress `Modal`, as the dashboard mounts it; anywhere else the hook's `closest()` finds nothing and the first measurement throws, which is why its own tests mock the hook rather than render it.
...
**The flame graph and that table talk through one ref.** [`RequestDetailView`](../src/overview/components/RequestDetailView.js) holds a `revealRef`, `LogEntriesTable` fills it with its own `reveal()` on mount, and the view passes `onRevealEntry` down through `RequestTrace` to `FlameGraph`. A Cmd/Ctrl+click on a frame therefore hands `reveal()` the position in the record of the log entry the frame opened at, `i`, which `Flame_Tree` stamps on every span it builds from a request's entries and `spliceFoldedSpans()` stamps on every stored row before it splices anything in, and the table goes straight to that row, unfolding it where it was collapsed and scrolling to it. Not the entry's `n`: a nested Perl render restarts it at 1 under the same request id, so numbers repeat within one record. Only a frame carrying no position — every frame of a request folded under load, whose tree `Flame_Fold` built — is resolved by its root-first path, keyed by each span's name and label along the spine. That path is a reconstruction, and it has broken with every change to what a span's start entry carries; the position is the identity the entry already has, which is why it leads. The other two callers of `RequestTrace`, the URL modal's aggregate flame and the current-request overlay tab, pass no callback, so a modified click does nothing there.
...
[`src/components/`](../src/components) holds the chrome a standalone page puts around its tree. [`DashboardShell`](../src/components/DashboardShell.js) is the fixed full-viewport box the Performance Overview, Gyroscope, Request Log and Error Log share. WordPress lays an admin page out in a padded max-width column, which cramps a wide monitoring surface, so the box is positioned rather than flowed: `top` clears the admin bar and `left` tracks the admin menu's live width through the substrate's `useAdminMenuWidth`, so folding the menu slides the page instead of reflowing it. The HEADER sits outside the scroller: it is chrome rather than content, so it holds its place while the page moves and the scrollbar runs beside the rows instead of through the header's own band. The box therefore clips, and the area below the header is what scrolls. `overflowY` is a required prop rather than a default and applies to THAT area — a dashboard whose body owns its own scroller must clip there, one without an inner scroller must scroll there, and getting it wrong is a double scrollbar or a truncated page. Both boxes are flex columns, because every dashboard root declares `flex: 1 1 auto; min-height: 0` and grows into what the header leaves. `overflowX` is not the caller's to choose: the box always clips it, so anything wider scrolls in its own container. `ThemedRoot` wraps that box rather than sitting inside it, because `DebugOverlay` mounts a skin provider of its own only when it finds no `.newspack-nodes-ui` ancestor, and the wrapper is that ancestor.
...
**The target is the scope.** The picker collects the descriptor chain from the clicked element outward, innermost first, so DOM nesting supplies the containment and the descriptor vocabulary needs no second attribute for scope. Six surfaces stamp it: `url:` on a `UrlTable` row and on the `UrlDetailView` root, `request:` on a `UrlDetailView` row and on the `RequestDetailView` root, `span:` on each `FlameGraph` frame (re-applied after every render, since d3 rebuilds the frames), `entry:` on a `LogEntriesTable` row, and `category:` in `RequestProfile`. Each pick sends its chain plus the dashboard's server filter to the `performance` CI's `ask` verb, which resolves the target in its container: a span or a category under a `request:` answers from that request, under a `url:` from that URL's aggregate flame or profile, and a category under neither from the leaderboard; a request picked under a `url:` carries a second `fetch` pointer to the URL. Every reply appends rather than superseding — a multi-select has several asks in flight at once, and the earlier answer is one the operator asked for. Three `<AskButton>`s open the mode, at the overview header and the two modal headers, and `<AskPanel>` renders what arrived.
