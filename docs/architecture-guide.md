...
| `eln-urls-page` | Two entries, both through `Performance_CI_Node::cached()`. One `urls` page per filter set, keyed by an md5 of every filter plus the window bucket, the retention and the store count, so every tab polling the same page shares one build a minute; and one `header` entry per scope, holding the `rows`, `totals`, `slowest` and `has_split` a ranked page still takes from the fold, keyed by the scope, the window bucket, the retention and the store count, and held `Stats_Store::BUCKET_SECONDS` — the bucket's own life — so the thirty-second fold runs once per five minutes while the ranked lists answer the table every poll. Its own Table outside the `evlog:p{N}` grammar, like `eln-rule-hooks`: a page folds every partition. A page over `URLS_PAGE_CACHE_MAX_ROWS` (250) rows, or a build the mirror read budget cut short, is served and not stored | 60 s, and 300 s for a header |
