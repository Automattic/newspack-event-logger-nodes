...
`set_stats_target <eln:stats_mirror_node>` mirrors the memcache stats into the durable `flame-stats:partition`, which a memcache miss reads back through its `stats-index` companion index; set `stats_mirror_node` to `''` to disable it. The mirror keeps the aggregates and both per-URL time series in full; the flame profiles alone are rank-capped, and only when `set_flame_topn` raises the default 0. `STATS_MIRROR_TOPN` is what an excluded namespace is spelled as, and it excludes the three derived hourly tiers, each answered from fine buckets the mirror already keeps. What bounds the buffer instead is `MAX_HELD_FRAMES` (10,000 a namespace), whose overflow is written to the partition early rather than dropped.
...
the other ([decision 14](architecture-decisions.md#decision-14-a-server-scope-the-key-cannot-carry-rides-inside-the-value-and-is-applied-as-a-projection)). Extremes are absent from
...
| `current-request` | `CurrentRequestTab`, registered through `registerDevtoolsTab` | its own batched-poll slice on `dump_request` |
