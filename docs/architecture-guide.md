...
**The flame graph and that table talk through one ref.** [`RequestDetailView`](../src/overview/components/RequestDetailView.js) holds a `revealRef`, `LogEntriesTable` fills it with its own `reveal()` on mount, and the view passes `onRevealEntry` down through `RequestTrace` to `FlameGraph`. A Cmd/Ctrl+click on a frame therefore hands `reveal()` the number of the log entry the frame opened at, `n`, which `Flame_Tree` stamps on every span it builds from a request's entries, and the table goes straight to that row, unfolding it where it was collapsed and scrolling to it. Only a frame carrying no number — every frame of a request folded under load, whose tree `Flame_Fold` built — is resolved by its root-first path, keyed by each span's name and label along the spine. That path is a reconstruction, and it has broken with every change to what a span's start entry carries; the number is the identity the entry already has, which is why it leads. The other two callers of `RequestTrace`, the URL modal's aggregate flame and the current-request overlay tab, pass no callback, so a modified click does nothing there.
