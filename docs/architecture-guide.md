...
| `eln-urls-page` | One folded `urls` page per filter set, keyed by an md5 of every filter plus the window bucket, the retention and the store count, so every tab polling the same page shares one fold a minute. Its own Table outside the `evlog:p{N}` grammar, like `eln-rule-hooks`: a page folds every partition. A page over `URLS_PAGE_CACHE_MAX_ROWS` (250) rows, or one the mirror read budget cut short, is served and not stored | 60 s |
