 ...
 * move together — `A` and then `I` each shipped writable but unreadable while
 * each component kept its own list. `errorStatus.test.js` parses the PHP
 * constant and fails when they disagree.
 *
 ...
 *
 * `tone` is the shared status modifier the sites append to
 ...
 *
 * @param {?string} code The record's `error_status`.
 * @return {?{label: string, tone: string}} Its entry, or null for a clean
 *                                          finish (`-`, '') and for any code
