/**
 * requestlog:stream tests — the SSE-in node that owns the live connection to the
 * completed-request firehose. `subscribe()` connects an SSE source; each inbound
 * `msg` envelope is emitted through the ONE sink (the exospine CI) stamped TO the
...
// A stream wired sink → a capture array, target → 'requestlog:route'.
function makeStream( fake, got ) {
	const s = createRequestLogStream( 'requestlog:stream', {
		connector: fake,
	} );
	s.sink = { fill: ( m ) => got.push( m ) };
	s.target = 'requestlog:route';
	return s;
}

describe( 'requestlog:stream', () => {
	test( 'emits one message envelope per SSE msg event to its sink, stamped TO the route', () => {
		...
		expect( got[ 0 ][ TO ] ).toBe( 'requestlog:route' );
	} );

	test( 'subscribes to the completed firehose', () => {
		const fake = makeFakeConnector();
	...
	test( 'names the node', () => {
		...
		expect( s.name ).toBe( 'requestlog:stream' );
	} );
} );

// The DEFAULT connector (no injected connector): EventSource + heartbeat + backoff.
describe( 'requestlog:stream default connector', () => {
	...
	test( 'opens a real EventSource at /messages/stream for the completed subscription', () => {
		const s = createRequestLogStream( 'requestlog:stream' );
		s.subscribe();
		const es = FakeEventSource.last();
		expect( es.url ).toContain( 'newspack-nodes/v1/messages/stream' );
		expect( es.url ).toContain( 'subscribe=completed' );
