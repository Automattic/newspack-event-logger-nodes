...
// The errors firehose subscription this dashboard streams.
...
class PerfErrorsStreamNode extends Node {
...
/**
 * Create and register the Error Log stream node.
 *
 * @param {string} name             Node name.
 * @param {Object} [opts]           Options.
 * @param {Object} [opts.connector] Injectable connector seam (connect/close);
 *                                  defaults to the real-EventSource connector.
 * @return {PerfErrorsStreamNode} The stream node.
 */
export function createPerfErrorsStream( name, opts = {} ) {
	const connector = opts.connector || makeDefaultConnector();
	const node = new PerfErrorsStreamNode( connector );
	node.setName( name );
	return node;
}
