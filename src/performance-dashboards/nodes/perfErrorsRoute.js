/**
 * `perferrors:route` — the classifier that makes the data/control split a
 * first-class, inspectable node instead of a bespoke `controlSink` edge.
 *
 * It receives everything `perferrors:stream` emits (sink = the CI, like every
 ...
 * a streamed errors-feed envelope may legitimately carry a `VALUE.action` field,
 * so keying on VALUE would misroute real data. Its data target is the node
...
// Wire errors-feed envelopes carry a rid / 'connected' KEY, never this.
const CONTROL_KEY = 'connection';

class PerfErrorsRouteNode extends Node {
...
/**
 * Create and register the Error Log route node.
 *
 ...
 * @return {PerfErrorsRouteNode} The route node.
 */
export function createPerfErrorsRoute( name, { dataTarget, controlTarget } ) {
	const node = new PerfErrorsRouteNode( dataTarget, controlTarget );
	node.setName( name );
	return node;
}
